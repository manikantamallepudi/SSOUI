...
export class LoginComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.logout != undefined){
      ...
      this.commonServices.getLogout(obj).subscribe(res => {
        if (res['success'] == 1) {
      ...
      res => {
        if (res['success'] == '1') {
         ...
         console.log( localStorage.setItem('currentUser', JSON.stringify(res)));
          ...
          window.location.href = "http://localhost:9201/reload?code=" + app_id + "&response_type=" + btoa_data + "&redirect_uri=" + this.GlobalServices.parentDomain;
